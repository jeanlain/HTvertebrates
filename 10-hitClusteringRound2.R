...
#we apply criterion 1 to hits from different communities within the same super
#family and MRCA (a "group"). The procedure is similar to what we did in the
#previous step
...
nCom = sapply(hitList, function(x)
  length(unique(x$com)))
...
#A and C belong to one of the 2 subclade diverging from the MRCA. Remember that
#we took care to place species of one subclade in column sp1 and species of the
#other subclade in column sp2, for each mrca.
...
quant = rbind(quant, data.table(
  clade = 0L,
  min = 0,
  q05 = 0,
  m = 0,
  divTime = 0
))			#we add this last row to ensure that the clades not in the quant table (those that were too young) will have Ks stats of zero. (see how the "nomatch" argument of match() below is set)
...
stats[, crit2 := (pmin(Ks1, Ks2) >= pmin(Ksc1, Ksc2) &
                    pmax(Ks1, Ks2) >= pmax(Ksc1, Ksc2))]    #applying criterion 2
...
########### their protein regions. For this we reuse the blastx results of
########### copies against repeat proteins (done in step 6-filterTEhits.R)
...
system(
  "blastp -query TEs/blastp/involvedProtOCC200Ks05.fas -db TEs/blastp/involvedProtOCC200Ks05.fas -max_target_seqs 1500 -outfmt 6 -num_threads 5 -evalue 1e-4 -out TEs/blastp/involvedProtOCC200Ks05.self.out"
)		#we set a larger max_target_seqs than the default to ensure that all possible hits are reported
...
blastp = fread(
  "TEs/blastp/involvedProtOCC200Ks05.self.out",
  header = F,
  col.names = c(
    "query",
    "subject",
...
#In theory, they should be all be there already since we did a self-blast. But
#some reciprocal hits are not found for some reasons, and some may have
...
nr = nrow(protRegions)
rows = 1:(nr - 1L)				#we will generate every possible pair of rows from this table in order to compare protein regions with each others and assess how they overlap
pairs = data.table(V1 = rep(rows, nr - rows), V2 = unlist(lapply(2:nr, function(x)
  x:nr)))		#this takes a lot of ram as there are lots of possible pairs
...
pairs = pairs[pair %in% stats$pair]
gc()

regionComp = pairs[, data.table(pair, protRegions[V1, .(prot, start, end)], protRegions[V2, .(prot2 =
                                                                                                prot,
                                                                                              start2 = start,
                                                                                              end2 = end)])]			#gets coordinate of protein regions we want to determine the homology
regionComp = proteinOverlap(regionComp, blastp, protSeqs)		#see function in HTvFunctions.R for details

compStats = regionComp[, .(
  max = max(interAll),
  sum = sum(interAll),
  maxL = max(length, na.rm = T),
  maxID = max(pID, na.rm = T)
), by = pair]				#for a pair of communities, gets the longest inferred alignment and total length of all
mStats = merge(stats, compStats, by = "pair", all = T)															#merges these with stats
mStats[com1 != com2 &
         is.na(max), c("max", "sum", "maxL", "maxID") := 0L]										#if a community was not even present in the protRegions, the values are NAs. We put zeros (we leave NAs for t1 == t2 as there was nothing to compare)
...
comStats = protRegions[, .(
  ...
  lr = max(end - start + 1),
  lp = max(lp)
), by = commID]				#number of different proteins, max protein length, etc.
...
hitGroups = mStats[, cLinkFromPairs(com1, com2, linked = crit2 &
                                      crit1)]						#does the clustering
mStats[, c("hitGroup1", "hitGroup2") := .(hitGroups[as.character(com1)], hitGroups[as.character(com2)])]
mStats[hitGroup1 == hitGroup2, all(crit2 &
                                     crit1)]													#checks that all pairs of communities belonging to the same group passed both criteria
