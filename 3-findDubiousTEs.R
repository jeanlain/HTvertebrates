...
consensusFiles = list.files(pattern = "families.fa$",
                            full.names = T,
                            recursive = T)
...
blast_nr = fread(out,
                 header = F,
                 sep = "\t",
                 col.names = fields)		#imports results
...
combined = combineHits(
  ...
  maxDiff = 300,
  blastX = T
)		#combining relatively adjacent hits (with a lot of margin). This function also renames columns
...
combinedRep = combineHits(
  ...
  maxDiff = 100000,
  blastX = T
)		#here we combined all hsps of each consensus, regardless of the protein (not very subtle, but it simplifies a lot)
...
dubious = function(occ) {
  merged = merge(
    ...
    suffixes = c("", ".r"),
    all = T
  )		#confronting the hit on repbase to the hit on nr for each consensus
  ...
  m = merged[is.na(subject.r) |
               leading > 90 |
               trailing > 90]						#we retain pairs of hits where there is no alignment on repbase, or where there are parts aligning on nr protein not covered by alignment on rep proteins (of at least 90 bp)
  m
}
...
dubiousConsensusHits = dubiousConsensusHits[!grepl(
  "transpo|retro|reverse|integrase|gag|pol-|pol |rna-dependent|polyprot|mobile|jockey|jerky|setmar|copia|recombinase|crypton|mariner|tcmar|tc1|gypsy|helitron|harbi|piggy|maveri|polinton|academ|ltr|cmc|envelop",
  subject,
  ignore.case = T
) &
  abs(qStart - qEnd + 1) > 100, ]		#retains hits to nr proteins that are not annotated as repeat proteins and with alignment length >100
...
combinedP = combineHits(blastp,
                        maxDist = 1000,
                        maxOverlap = 100000,
                        maxDiff = 1000)
repProtAccessions = combinedP[abs(qEnd - qStart) + 1 >= 100 &
                                identity >= 35, unique(splitToColumns(query, " ", 1))]		#gets accession number of nr proteins that have an acceptable hit with a rebpase proteins. We will consider them as legit TE proteins
