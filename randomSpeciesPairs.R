...
HTThits = retainedHits[independent == T, .(
  ...
  repl = 0L
), by = .(hitgroup, superfamily)]		#repl is a replicate number (= 0 for original, non-permuted species)
...
hitList = split(HTThits[, .(sp1, sp2, repl)],
                f = list(HTThits$superfamily, HTThits$batch),
                drop = T)		#splits the hits by superfamily and batch
...
newsp = matrix(rep(1:l, 10 ^ 5), ncol = 10 ^ 5)		#this matrix will contain the permuted "replacement species" during the work, one set of species (permutation) per column. 
#Note that we anticipate 10^5 permutations per batch, although we keep fewer. This is because many permutations may be "illegal". 
#The row indices of this matrix = species (integer ids)


shuffle = function(hits, superfamily) {
  #permutates species for htts (hits) of a superfamily
  ...
  shuffleWork = function(job)	{
    #performs the permutations in parallel. job is a simple integer identifier
    ...
    newSp = as.vector(legal[, 1:maxn])			#"unrolls" the matrix of legal permutations (useful to replace species with the sampled ones,
    #as the position in this vector will be the original species identifier, and its value is the replacement species). 
    #We do not retain more than maxn legal permutations (there may actually be up to 10^5 if there are few hits).
    
    repl = rep(1:maxn, each = nr)				#generates a permutation id number
    pairs = data.table(
      sp1 = rep(hits$sp1, maxn) + (repl - 1L) * l,
      sp2 = rep(hits$sp2, maxn) + (repl - 1) * l,
      repl
    )	#we replicate the transfers n times but incrementing species numbers, so that we can easly perform the replacements with random species. 
    #This is similar to what we did to create the newPairs matrix, except this time we use a data table
    
    pairs[, c("sp1", "sp2", "repl") := .(newSp[sp1], newSp[sp2], repl + job *
                                           maxn)]		#we use the job id to generate final replicate identifiers, which will have to differ between jobs
    pairs
  }
  pairs = mclapply(1:nCPUs - 1L,
                   shuffleWork,
                   mc.cores = nCPUs,
                   mc.preschedule = F)	#does the permutations in parallel
  rbindlist(pairs)
}

randomHTTs = Map(shuffle, hitList, names(hitList))		#applies permutations to superfamilies successively

for (i in 1:length(randomHTTs)) {
  #We generate unique repliate identifiers within eahch super family
