...
##%######################################################%##
#                                                          #
####  This stage computse Ks between TEs for selected   ####
####   TE-TE hits, to select hits that represent HTT.   ####
#                                                          #
##%######################################################%##


#We need to extract copy sequences for that (they were actually not extracted in the blastx we preformed prior, as we piped directly from seqtk to diamond)
...
m = Map(function(x, y)
  write(x, file = y), spl, fileNames)			#write copy names for each species so we can use seqtk to extract them from fastas
...
seqs = mcMap(seqtk,
             ...
             fileNames,
             mc.cores = 20,
             mc.preschedule = F)		#ingests selected copies from TE fastas in parallel
...
blastx = blastx[eValue <= 0.001 &
                  ex == 2L]						#selects hits between TEs and rep protein of the same super family, and with sufficient evalue
blastx[, c("start", "end") := .(qStart, qEnd)]
blastx[qEnd < qStart, c("start", "end") := .(qEnd, qStart)] 	#creates new start and end coordinates so that the former is always lower
...
system(
  "Rscript TEKaKs.R allOCC2000.txt TEKs/blastxOCC2000EXcov1.out TEKs/selectedCopiesAA300cl2000.fas TEKs/ 30"
)
...
httHits = selectedHits[ks + 2 * vks < qKs &
                         length.aa >= 100L &
                         ks < 0.5, ]			#selecting hits that should constitute HTT, according to our criterai (see Methods section). The removal of all hits between species that diverged in the last 120 My was done quite late in the pipeline (doing it here would save some computationnal time, but won't change the results)
