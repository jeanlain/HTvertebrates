...
# we import results from ka ks value between TEs within communities generated by step 13-TEKaKsWinthinGenomes.R. 
...
# We visually assess how the genetic distance between the two TEs of a pair 
# and the difference in GC content in TEs impact the Ka/Ks of this pair 
...
# So we consider Jockey as a subclass for these plots
...
perSubclass  <- split(
  allKaKs[,.(ka, ks, nMut, K80distance, GCdifference, GCcontigDifference, subclass, htt)], 
  f = allKaKs$subclass) 

# the figures only change in respect to the X axis (factor we investigate)
...
names(xNames) <- names(perSubclass[[1]])[4:6]

# We generate each of the 3 supplementary figures,
for (i in 1:3) {
  ...
  for (subclass in names(perSubclass)) {
    ...
    # we compute weighted means and standard errors per class and separating
    # copies that diverged through HTT or not
    perClass <- copyPairs[ks > 0, .(
      meanKaKs = weighted.mean((ka - ks) / (ka + ks), w = nMut, na.rm = T),
