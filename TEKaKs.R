##%######################################################%##
#                                                          #
#### This stage computes Ka/Ks  between homologous TEs  ####
#                                                          #
##%######################################################%##
...
hits = TEhits[, which(query %chin% covPerCopy[V1 > 30L, copy] &
                        subject %chin% covPerCopy[V1 > 30L, copy])]		#hits are simply row indices of TEhits table
...
hits = splitEqual(sample(hits), n = nJobs)																	#splits the hits in batches for parallel computations. 
#Note that we randomize hits (rows) to reduce differences in job durations (the longer hits are on top of the TEhits table)
...
qSeqs = lapply(hits, function(x)
  qSeqs[x])
sSeqs = lapply(hits, function(x)
  sSeqs[x])
TEhits = lapply(hits, function(x)
  TEhits[x, .(query, subject, qStart, qEnd, sStart, sEnd)])
rm(seqs, covPerCopy)
gc()

TEKs = function(job) {
  #this computes Ks on a batch of hits. job is the job number
  ...
  nuc = nuc[!is.na(hit1) &
              !is.na(hit2)]																	#we can already remove positions outside protein HSPs
  ...
  #we now compute Ka and Ks values with seqinr. for this we first discard
  #incomplete codons from alignments, as we will concatenate nucleotides back
  #into sequences. Codons will be identified by integers we determine whenever a
  #new codon starts (jumps in position within codon, new alignment, indel)
  n = 2:nrow(nuc)
  shift = nuc[, c(T, (
    abs(frame1[n] - frame1[n - 1]) != 1L |
      aln[n] != aln[n - 1L] |
      abs(pos1[n] - pos1[n - 1L]) != 1L |
      abs(pos2[n] - pos2[n - 1L]) != 1L
  ))]
  ...
  flat = nuc[, .(
    ...
    forward = frame1[1] > 0L
  ), by = aln]	#we now concatenate the positions backs into strings. We also note if the copies are in reverse in respect to the protein (frame is negative)
