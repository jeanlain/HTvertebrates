...
system('sbatch --mail-type=BEGIN,END,FAIL --cpus-per-task=20 --mem=20G --wrap="Rscript randomSpeciesPairs.R 20"')		#performs the permutations on 20 CPUs
...
countHTTs = function(superfamily) {
  #counts htt per taxon in a given TE super family (or batch within a larger superfamily) to obtain stats (mean, quantiles) for simulated transfers
  ...
  missingTaxa = data.table(
    ...
    N = 0
  )		#this table is required to add 0 counts for taxa not involved in HTT in each replicate 
  ...
  stats = counts[repl > 0L, .(
    ...
    minNumber = min(N),
    maxNumber = max(N),
    qLc = quantile(N, 0.005),
    qRc = quantile(N, 0.995),
    qL5 = quantile(N, 0.025),
    qR5 = quantile(N, 0.975),
    superfamily
  ), by = taxon]		#obtains the statistics over permutations for simulated data
...
stats[, class := ifelse(
  grepl(
    "CMC|hAT|Mariner|Maverick|Merlin|PIF|PiggyBac|DNA",
    superfamily
  ),
  "DNA",
  "RNA"
)]		#attributes TE class (note that many of these families are not even in the table, but they could in other situations)
...
linkedPlots = function(dt,
                       space = 2,
                       legend = F,
                       ...) {
  #generates the linked barplot figure. Almost all the code adds stars for siginificance levels
  b = dt[, linkedBarPlot(
  ...
  dt[observed < qL5 |
       observed > qR5, stars := "*"]		#difference at the 5% level
  dt[observed < qLc |
       observed > qRc, stars := "**"]		#at the 1% level
  dt[observed < minNumber |
       observed > maxNumber, stars := "***"]  	#at the 1/1000 level
  f = dt$stars != ""			#TRUE when there are significant differences
  labels = dt[f, ]
  y = cumsum(c(0, dt$observed))			#below, we compute the location of the stars, and the segments linking them to the barplot sections. 
  #This can be tricky for small barplot sectors, as we need to avoid collisions.
...
aquatic = c(tipsForNodes(tree, c(376, 472, 474, 514, 501, 463))$tip,
            grep("Trichechus", tree$tip.label))	#aquatic species among tetrapods
...
habitatEffect = function(tetrapods, aquatic) {
  ...
  tetraHits = HTThits[(sp1 %in% tetrapods &
                         sp2 %in% fishes) |
                        (sp2 %in% tetrapods &
                           sp1 %in% fishes)]		#hits between a tetrapod and a non-tetrapod
  ...
  sp1 = permu[sp1]
  pairs = rbind(data.table(sp1 = tetraHits$sp1, repl = 0L),
...
HTThits = retainedHits[!duplicated(hitgroup), data.table(sp1 = chmatch(sp1, tree$tip.label),
