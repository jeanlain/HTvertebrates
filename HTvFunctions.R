...
rescale = function(x, range) {
  #rescales a numeric vector so that its range (min and max) corresponds to the range argument (a 2-number vector)
...
reList = function(spl, len = NA) {
  #rearranges a list whose names are integers so that the names of elements of the list are placed at equivalent indices of a new list.
...
occurences = function(table) {
  ...
  # gets the occurence of each combination of elements at each row
  dt = data.table(table)
  dt[, pos_ := 1:.N]
...
assignToRegion = function(bed, pos, olv = T) {
  ...
  scaffLengths = rbind(bed[, .(scaffold, pos = end)], pos)[, max(pos) + 1L, by =
                                                             scaffold]					#we convert coordinates into global genome coordinates to call .bincode. 
  #This requires estimating the length of scaffold, which is the max coordinate per scaffold for the bed and pos tables combined. "+1" gives a bit or room (probably not needed).
  starts = scaffToGenomeCoord(bed$scaffold,
                              bed$start,
                              scaffLengths$V1,
                              scaffLengths$scaffold)
  pos = scaffToGenomeCoord(pos$scaffold,
                           pos$pos,
                           scaffLengths$V1,
                           scaffLengths$scaffold)
  dt = data.table(boundary = c(starts, starts + bed[, end - start + 1L]),
                  id = rep(bed$id, 2))						#creates a data table with start and ends in the same "boundary" column. 
  #These will be intervals for binning. We use "id" to keep track of the rows (regions id) of the original bed
  setorder(dt, boundary)
  dt[, c("row", "region") := .(1:.N, as.integer(NA))]													#"region" will be the region (id) covering an interval (only for the start of this interval). 
  ...
  if (length(shorts) < nrow(dt)) {
    ...
    if (length(shorts) == 0) {
      ...
      rows = dt[-shorts, .(row = row[1L]:(row[.N] - 1L), size = row[.N] - row[1L]), by = id]				#for each of these overlaping regions, we create a vector that represent all the rows of dt that are covered by that region
    setorder(rows,-size)																			#puts the longest regions on top
    dt[rows$row, region := rows$id]																	#so we can add a region id for each row in dt.
...
genomeCov = function(bed,
  ...
  dt = bed[, data.table(
    scaffID = rep(toInteger(scaffold, unique = F), 2),
    pos = c(start, end + 1L),
    n = rep(c(1L, -1L), each = .N)
  )]			#"melts" the bed to obtain 2 rows per read: alignment start and alignment end + an index (n) of 1 for starts and -1 for ends. We also converts scaffold names to integers to save memory. We also add 1 to end positions, as this will facilitate merging intervals
  ...
  dt = dt[!duplicated(data.table(scaffID, pos), fromLast = T)]		# reads starting or ending at the same position creates duplicate positions. 
  #We only retain the last row among each set of duplicates as this is the one with accurate depth
  if (combine)
    dt[cov > 1L, cov := 1L]	#to combine regions with at least 1X depth, we convert all depth > 1 into 1
  
  if (successive) {
    dt = dt[c(T, diff(scaffID) != 0L |
                diff(cov) != 0L)]  #we combine successive rows in the same scaffold with identical coverage 
    #(may happen if a read ends just before another read starts). This effectively combines region with ≥ 1X if combine == T
  }
  nr = 2:nrow(dt) - 1L
  res = dt[, data.table(
    scaffID = scaffID[nr],
...
compl = function(bases) {
  ...
  chartr("acgtmrwsykvhdbACGTMRWSYKVHDB",
...
allPairs = function(v,
  ...
  if (all(is.na(v2)) & !reciprocal) {
    ...
    if (all(is.na(v2)))
      v2 = v
    self = T
    cb = data.table(V1 = rep(v, each = length(v2)),
                    V2 = rep(v2, length(v)))
    if (!self & reciprocal)
      cb = rbind(cb, cb[, 2:1])
    cb
...
combineHits = function(blast,
  ...
  queryRegions = combineRegions(blast[!is.na(group), data.frame(group, qStart, qEnd)], distance = maxDist *
                                  2)
  subjectRegions = combineRegions(blast[!is.na(group), data.frame(group, sStart, sEnd)], distance = maxDist *
                                    2)
  ...
  combined = data.table(regionStats[, -1, with = F], queryRegions[, -1, with =
                                                                    F], subjectRegions[, -1, with = F])
...
cLinkFromPairs = function(V1, V2, linked) {
  ...
  if (is.integer(V1))
    ...
    i1 = chmatch(V1, u)					#convertion to integer number
  if (is.integer(V2))
    i2 = match(V2, u)
  else
    i2 = chmatch(V2, u)
  ...
  set = split(allReciprocalPairs[, 1], allReciprocalPairs[, 2])							#so the splitting yields, for each element, a "set" (=value of the list) of all the elements 
  ...
  for (i in w) {
    ...
    if (!t2 %in% clique[[t1]]) {
      ...
      if (all(clique[[t2]] %in% set[[t1]])	 &
        ...
        set[newGroup] = list(intersect(set[[t2]], set[[t1]]))				#then each element of the new clique has its set become the intersection of the t1 and t2 sets, i.e, 
...
seqtk = function(fas,
  ...
  #Frite to fasta file out (if specified) or return to R (if not). if formated is TRUE, return sequences as a DNAStringset (if not, retursn a character vector corresponding to the fasta) ex specifies how seqtk subseq should be executed
  if (missing(out)) {
...
extractCopies = function(sp, gff, cons, genome, out, minLen = 300L) {
  ...
  selectedCopies = gff[grepl("/", family, fixed = T) &
                         V5 - V4 + 1L >= minLen]	#selects TE copies that are long enough and assigned to a precise super family (which has a slash in its name, like "DNA:Tc1/Mariner")
...
getBUSCOs = function(folder) {
  #concatenates busco genes of a species (having a specific folder) in a single fasta and renames sequences by adding the species name to each
...
translateCDS = function(cds, aa) {
  ...
  data.table(cds = basename(cds),
             nseq = length(seq),
             stops = sum(stops > 0))	#returns some reports regarding number of sequences with internal stop codons
}



compute_Ks = function(pairs) {
  #computes neutral divergence (Ks) of a pair of species based on reciprocal blastp results (one species vs another)
  require(seqinr)
  forward = lapply(pairs$forward, function(x)
    suppressWarnings(fread(x, header = F, sep = "\t")))		#imports forward blastp search
  reverse = lapply(pairs$reverse, function(x)
    suppressWarnings(fread(x, header = F, sep = "\t")))
  ...
  setnames(forward, colNames)
  setnames(reverse, colNames)
  ...
  RBH = RBH[qEnd <= nchar(allAA[query]) &
              sEnd <= nchar(allAA[subject])]					#because there appears to be a bug with diamond where rare hits have coordinates longer than the query or subject
  ...
  if (nrow(RBH) > 0) {
    ...
    res = data.table(
      RBH[, c(1, 2, 7:10), with = F],
      Ks = sapply(KaKs, function(x)
        x$ks),
      Ka = sapply(KaKs, function(x)
        x$ka),
...
expectedProtein = function(query, subject) {
  #determines whether a TE copies has a hit against a repBase protein of the same super family. This is a bit tricky since the super family information has no standard format in repbase
  ...
  test = rowSums(superFam == superFamProt[, 1:min(ncol(superFam), ncol(superFamProt))], na.rm =
                   T)					#determines the number of elements that are identical between superFamily names (e.g., DNA + TCMAR + Mariner)
  ...
  exp[diff > 0 &
        test >= 2] = 1														#set to 1 when 2 elements are the same, but not all (ex DNA-TCMAR-MARINER vs DNA-TCMAR-TC1 or just DNA-TCMAR).
  exp
}


processBlastX = function(blast) {
  #processes results of a blastx of TE copies against repeat proteins, see below
  ...
  if (ncol(fields) == 7) {
    #detects whether copy parts and not full copies (first round) were blasted, based on the number of fields in the sequence names
...
unalignedParts = function(blast) {
  ...
  selectedCopies = blast[ex > 0 &
                           length >= 30, unique(query)]					#we do it for copies that had an acceptable HSP (=expected protein family and at least 30 aminoacids aligned), since any other hit should be of lower quality (we set max_taget_seqs 1, so we have the best hit possible).
  ...
  leading = coords[, data.table(copy = query,
                                start = 0,
                                end = start - 1)]		#unaligned leading part of each copy
  trailing = coords[, data.table(copy = query,
                                 start = end,
                                 end = copyEnd)]
...
nestedClades = function(tree, symmetrical = T) {
  ...
  childs = lapply(nodes, function(x)
    subNodes(tree, x))
  childOf = cbind(parent = rep(nodes, sapply(childs, length)),
                  child = unlist(childs))
...
tipsForNode = function(tree,
                       ...
                       names = F,
                       itself = F) {
  #returns all tips for a node from tree, or NULL if node is a tip (except if itself is TRUE). Returns tip names if names is TURE, else tip numbers
  tips = NULL
  n = node
  while (length(n) > 0) {
...
tipsForNodes = function(tree,
                        ...
                        names = F,
                        itself = T) {
  #returns all tips of several tree nodes, based on the function above. returns a data.table whose firt column is the node number and second the tips
...
combineHomologous = function(protRegions, blastp, protSeqs) {
  ...
  repeat {
    ...
    allP = allP[, data.table(commID, protRegions[V1, .(prot, start, end)], protRegions[V2, .(prot2 =
                                                                                               prot,
                                                                                             start2 = start,
                                                                                             end2 = end)])]				#gets  protein names and region coordinates for these pairs
    ...
    allP[, cov := intersection(start, end, qStart, qEnd, negative = F) / (end -
                                                                            start + 1)]									#cov = the proportion of a region that is covered by the alignment with prot2
    ...
    sel[, c("newStart", "newEnd") := .(corres[chmatch(startID, id), pos2] - (mini -
                                                                               start), corres[chmatch(endID, id), pos2] + end - maxi)]		#coordinate conversion
...
proteinOverlap = function(regionComp, blastp, protSeqs) {
  ...
  toConvert = regionComp[, !is.na(sStart) &
                           prot != prot2 &
                           !is.na(inter1[, 1]) &
                           !is.na(inter2[, 1])]		#we only realign different proteins (not the same with itself) that are involved in blastp
  realign = blastp[pair %chin% regionComp[toConvert, pastePair(prot, prot2, sep = " ")]]
  aln = realign[, alignWithEndGaps(stri_sub(protSeqs[query], qStart, qEnd),
  ...
  regionComp$interAll = intersection(inter1[, 1], inter1[, 2], inter2[, 1], inter2[, 2], negative =
                                       F)						#intersection of the 2 intersections = the width of alignment/homology between the 2 regions
...
fadeTo = function(source, dest, amount) {
  #fades source colors into destination colors, by a certain amount (from 0 to 1). alpha is not managed and any transparancy is removed
...
arcSegments = function(x0,
                       ...
                       draw = T,
                       ...) {
  #draws arcs between points defined by theyr coordinates (first 4 arguments,
  #numeric vectors). The arc is drawn from the first to the second point
  #counterclockwise if the angle is > 0, and clockwise if the angle is > 0. nv
  #is the number of vertices in the line drawn. Silenlty returns a list of
  #points to pass to lines() or polygon() for the arcs. Only draws on the plot
  #if draw = T.
  dX = x1 - x0
  dY = y1 - y0
  ...
  xCoords = Map(function(x, radius, angle)
    x + radius * cos(angle), x, radius, angles)		#coordinates of the points of the line(s) to draw
  yCoords = Map(function(y, radius, angle)
    y + radius * sin(angle), y, radius, angles)
  
  if (draw)
    Map(lines, xCoords, yCoords, ...)
  dt = Map(function(x, y)
    data.table(x = x, y = y), xCoords, yCoords)
  invisible(dt)
}
...
roundedRect = function(x0,
  ...
  d[d == 0] = -1    #1 if descending slope, -1 if ascending or horizontal
  dX = abs(x1 - x0)
  dY = abs(y1 - y0)
  dy = dX * l / L
  dx = dY * l / L * d
...
linesFromTree = function(tree, angular = T) {
  ...
  for (i in 1:length(paths)) {
    ...
    x = X[nodes]
    y = Y[nodes]						#we get their coordinates
    if (angular) {
...
outlineTaxon = function(tree,
  ...
  if (abs(leftX - xRange[1] + xMargin) < 2)
    leftX = xRange[1] - xMargin 	#adjusts the left side of the rectangle to the left side the rectangle surrounding the clade, if it's close enough, for best visual effect
  if (abs(rightX - xRange[2] - xMargin) < 2)
    rightX = xRange[2] + xMargin	#same for the right side (considering that it's very unlikely that the left side was close enough in this case)
  ...
  rect = roundedRect(
    ...
    nv = 20,
    plot = F
  )		#gets xy coordinates of rounded rectangle around the label
