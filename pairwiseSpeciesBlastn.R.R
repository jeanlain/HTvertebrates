...
done = unique(list.files("TEs/blastn/done"))		#the output file names of jobs that are already done 
#(to avoid redoing the search, in case the job is relaunched due to a bug, memory issue, and so on)
...
blastn = function(fas, db, out) {
  ...
  #Filtering these hits from the begining was required, else we might have run out of disk space on our account
