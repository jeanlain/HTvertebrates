...
blastFiles = blastFiles[file.exists(blastFiles)] ; blastFiles = blastFiles[file.size(blastFiles) > 0] 
blast = rbindlist(lapply(blastFiles, function(x) fread(x, header =F, drop = 4, sep = "\t")))
blast[,V3 := as.integer(V3*1000L)]										#converts pID to integer as we did for the htt hits (to save memory)

hitCommunities = function(group) {										#we do the clustering per 'group' (hits of pair of clades in a super family)
	...
	sub = blast[!is.na(qid) & !is.na(sid)]							#extract only the hits we need in these
	...
	criterion_1 = function(batch) {			#this function "connects" hits according to criterion 1
		pairs = data.table(V1 = rep(batch, nHits-batch), V2 = unlist(lapply(batch[1]:max(batch)+1L, function(hit) hit:nHits)))					#for a batch of hits, we make pairs of hits. The left-hand hit (V1) is from the batch, and the right-hand hit (V2) includes all other hits (including other matches)
		...
		cat("*")
		pairs[inter1 < maxIntra | inter2 < maxIntra, maxIntra, .(V1,V2)]			#returns pairs of hits where best intra-clade identity is higher than one inter-clade identity (that of hits)
	}
	
	pairs = rbindlist(lapply(hitBatches, criterion_1))						#does the above for batches of hit pairs and concatenates the results
	if(nrow(pairs) > 0) {													#if there is not connected hits, there is not clustering to make
		...
		com = integer(nrow(hits)) ; com[cls$member] = cls$community			#we attribute hits to "communities"
	...
	writeT(hits[,.(hit, comm, group)],stri_c("TEs/clustering/round1/", group, ".groups.txt"))		#saves results to disk for safety
	hits[,.(hit, comm, group)]
}



res = mclapply(groups, hitCommunities, mc.cores = nCPUs, mc.preschedule = F)
res = rbindlist(res)
if(length(superF) > 1) superF = "others"
writeT(res, stri_c("TEs/clustering/round1/", superF, ".allGroups.txt"))
print("finished")
