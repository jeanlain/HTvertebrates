...
# This scripts is launched from 9-hitClusteringRound1.R and uses files generated by that script
...
blast <- rbindlist(lapply(X = blastFiles, FUN = function(file) {
    fread(
        input = file,
        header = F,
        drop = 4, # we do not need this column from the blast output 
...
hitCommunities <- function(group) {
    ...
    if (nrow(pairs) > 0) {
        ...
        # its own community number corresponding to its row index
        hits[, comm := 1:.N]
    }
