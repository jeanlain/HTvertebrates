...
# this script uses the fasta files of complete BUSCO genes in each species in a "BUSCO_results" folder (The BUSCO pipeline was not automated in scripts)
...
# STEP ONE, we peform diamond blastp search of BUSCO proteins to find orthologs between which we compute Ks, and to align proteins afterwards ---------------------------------------
# (we will use protein alignments as guides since Ka Ks are based on codons)
...
# we nos translate CDS into proteins ---------------------------
...
# translating the BUSO CDS into proteins in parallel jobs is done by the function below
...
# we stack the reports about stop codons in a single table
...
# we mmake diamond blastp databases, one per species --------------------------------------------
...
# and there is no need to measure Ks in all possible species pairs between two large clades (only a subset of species is sufficient)
...
# we retreive all these young clades with their species (see function in HTvFunctions.R)
...
# STEP TWO obtaining and processing Ks values from blastp results (with 30 CPUs) -----------------------------------------
...
# which we will use to establish the ditribution of Ks values for each clade
...
KsDistrib <- Ks[Ks < 9 &
    ...
    # ignoring Ks values ≥ 9 (oversaturated)
