######### computes Ks between species for core genes extracted by BUSCO, in order to compare Ks between TEs and core genes
...
m = mclapply(folders, getBUSCOs, mc.cores = 10, mc.preschedule = F)
...
translationSummary = mcMap(translateCDS, CDSfiles[f], aaFiles[f], mc.cores = 10, mc.preschedule = F)	#translation done in parallel
...
makedb = function(folder,db, fas) {				#makes a diamond db
	system(paste("diamond makedb --in", fas, "-d", db))
}
m = mcMap(makedb, folders[f], dbs[f], AAfiles[f], mc.cores = 10, mc.preschedule = F)  #databases generated in parallel
...
pairs =data.table(divTime = as.vector(distMat), sp1 = rep(colnames(distMat), nrow(distMat)), sp2 = rep(rownames(distMat), each = ncol(distMat)))
...
pairs = pairs[sp1 %chin% aaSpecies & sp2 %chin% aaSpecies]					#only retains pairs of species that have BUSCO genes
...
pairs = pairs[(divTime >= 500 & !sp1 %chin% toIgnore & !sp2 %chin% toIgnore)  | (divTime > 80 & divTime  <500)]
...
system('sbatch --mail-type=BEGIN,END,FAIL --cpus-per-task=30 --mem=50G --wrap="Rscript coreGenesblastp.R 1 30"')  # example of the 1st job with 30 CPUs
...
Ks = mclapply(list.files("Ks/KaKs", full.names = T), function(x) fread(x, header = F, sep = "\t"), mc.cores = 3)	#importing all results off KaKs computations
Ks = rbindlist(Ks)
setnames(Ks, c("query","subject","qStart","qEnd","sStart","sEnd","Ks","Ka","alnLength"))
...
Ks = Ks[alnLength > 200 & !duplicated(data.table(clade, subject)) & !duplicated(data.table(clade, query))]		#ignores alignments shorter than 200 aa and retains no more than one alignment per BUSCO gene per clade
