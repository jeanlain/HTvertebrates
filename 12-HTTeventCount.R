...
#                                                          #
##%######################################################%##
#(see the study's methods for details, as this would be quite long to summarize in a comments)
...
##### we need to find copies that are similar between hit groups to assess
##### whether they can be "exlpained" by others. We do it per superfamily
...
#First, we assess whether hitGroup2 involves species related to those of
#hitGroup1. We assess if a clade of hitGroup1 is the same as, nested in, or
#encompassing, a clade of hitGroup2. Note that this doesn't require that species
#are shared. "clade 1" is always the clade on the left of table, (hence
#containing the "query" copies) in the hit group hits
tree = read.tree("timetree.nwk")
clades = httHits[, .(node1 = MRCA(tree, unique(sp1)),
                     node2 = MRCA(tree, unique(sp2))), by = hitGroup]		#the pairs of clades involved in each hit group (transfer), encoded as node numbers in the species tree
...
node1 = clades[match(1:max(hitGroup), hitGroup), node1]
node2 = clades[match(1:max(hitGroup), hitGroup), node2]		#same principle as above for the 2 clades composing the hit groups. Doing this speeds up the command below as this avoid performing a match on the huge hitGroupPairs table
...
hitGroupPairs[, c("nested1", "nested2", "nestedMRCA") := .(mat[cbind(n1_1, n1_2)] |
                                                             mat[cbind(n1_1, n2_2)], mat[cbind(n2_1, n1_2)] |
                                                             mat[cbind(n2_1, n2_2)], mat2[cbind(n1_2, mrca1)] |
                                                             mat2[cbind(n2_2, mrca1)])] #these new colums will be TRUE if the clade1 of hitGroup1 is nested, or indludes, a clade of hitGroup2. nested2 does the same for clade2 of hitGroup1, and if the mrca of hitGroup1 is nested within a clade of hitGroup2 (in which case we will consider that hitGroup2 cannot exlpain hitGroup1)
hitGroupPairs[, c("n1_1", "n1_2", "n2_1", "n2_2", "mrca1") := NULL]


#We also impose that the best identity that a copy of hitGroup1 has with any
#copy of hitGroup2 is not lower that the lowest identity the copy has within
#hitGroup1
...
hitGroupPairs[, minID := minCopyIDs[match(copyHitGroup1, copyHitGroup), as.integer(V1 *
                                                                                     1000)]]		# this allows placing this pID in the hitGroupPair table
...
hitGroupPairs[, sp := temp[match(copy, cop), sp]]
rm(temp)

hitGroupPairs[, c("inClade1", "inClade2") := .(copyHitGroup1 %in% httHits[, q *
                                                                            10000 + hitGroup], copyHitGroup1 %in% httHits[, s * 10000 + hitGroup])]		#we also determine if a copy belongs to a species from the left or right clade for each hit group

# we also determine if hitGroup1 and hitGroup2 were not considered independant
...
# will decide that hitGroup2 cannot explain hitGroup1.
...
crit1[, pair := ifelse(hitGroup1 < hitGroup2,
                       hitGroup1 * 10000 + hitGroup2,
                       hitGroup2 * 10000 + hitGroup1)]
hitGroupPairs[, hitGroupPair := ifelse(hitGroup1 < hitGroup2,
                                       hitGroup1 * 10000 + hitGroup2,
                                       hitGroup2 * 10000 + hitGroup1)]
hitGroupPairs[, indep := hitGroupPair %in% crit1[V1 < 0.05, pair]]
rm(mStats, crit1)
...
#we now select rows passing filters. A copy of a hit group can be brougth by
#another HTT (hitGroup2) only if the hitGroup2 copy belongs to a species that is
#the same (or related) to the one carrying the copy. We also consider that
#hitGroup2 cannot explain hitGroup1 if this pair of hit groups did not pass
#criterion 1 or if the mrca of hitGroup1 is nested within a clade of hitGroup2
#We also discard hit groups that were considered unreliable in the previous
#step. This could have been done earlier, but we prefer applying at the last
#moment.
sel = hitGroupPairs[pID >= minID &
                      indep == F &
                      ((inClade1 == T &
                          nested1 == T) |
                         (inClade2 == T &
                            nested2 == T)) &
                      nestedMRCA == F &
                      hitGroup2 %in% orderedHitGroups &
                      hitGroup1 %in% orderedHitGroups, .(hitGroup1,
                                                         hitGroup2,
                                                         sp,
                                                         c1 = nested1 & inClade1,
                                                         c2 = nested2 & inClade2)]

#as we will evaluate whether hitGroup2 could have brought TE copies in species
#composing a clade of hitGroup1, we do the following: for each retained hit
#group, we list the species (for each clade) that have copies similar to those
#of other hit groups harboring related species. This is done in the function
#below
sharedSpeciesOfClade = function(clade) {
  #clade is either 1 (left clade) or 2
  if (clade == 1) {
  ...
  contributedClade = reList(split(temp$hitGroup1, temp$hitGroup2),
                            max(orderedHitGroups))	#so contributedClade[[n]] list the hit groups whose left-clade copies are "explained" or "contributed" by hit group #n (i.e., are similar to those of hit group #n).
  list(spList, contributedClade)		#we return both lists
}

res = sharedSpeciesOfClade(1)
clade1Sp = res[[1]]
contributedClade1 = res[[2]]
res = sharedSpeciesOfClade(2)
clade2Sp = res[[1]]
contributedClade2 = res[[2]]

#to evaluate whether a hit group can be adequately explained by others, we also
#need to list all the species pairs (constituting the hits) it involves. Species
#of the left clade are on the left in pairs. Pairs are encoded as integers, to
#speed up subsequent commands
temp = httHits[, unique(chmatch(sp1, tree$tip.label) * 1000L + chmatch(sp2, tree$tip.label)), by = hitGroup]
spPairsForHitGroup = reList(split(temp$V1, temp$hitGroup))
rm(temp)		#for each hit group (index of this list), gives the species pair
...
isExplained = function(hitGroup) {
  #tells if a focal hit group (integer number) can be explained, returns T or F
...
removeHitGroupFrom = function(hitGroup, ls) {
  #removes a hit group (integer) from a list (argument ls) of potential explanatory groups
  charHitGroup = as.character(hitGroup)
  lapply(ls, function(x)
    x[!names(x) %chin% charHitGroup])
}
...
for (i in 1:length(orderedHitGroups)) {
  #takes hit groups by order or reliability
  hitGroup = orderedHitGroups[i]
  if (isExplained(hitGroup)) {
    #if a hit group can be explained by others
    expl1 = intersect(which(explained |
                              required), contributedClade1[[hitGroup]])		#retreives the already explained or required hit groups that are partly explained by this hit group, for the left clade
    expl2 = intersect(which(explained |
                              required), contributedClade2[[hitGroup]])		#same for the right clade
    expl = union(expl1, expl2)
    if (any(expl)) {
      #if there are any
      clade1Spb = clade1Sp
      clade2Spb = clade2Sp			#makes backups of the lists we use (we could do differently, but this allows reducing the number of arguments passed to isExplaiend())
      if (any(expl1))
        clade1Sp[expl1] = removeHitGroupFrom(hitGroup,  clade1Sp[expl1])		#and removes the hit group from the explanatory ones (left clade)
      if (any(expl2))
        clade2Sp[expl2] = removeHitGroupFrom(hitGroup,  clade2Sp[expl2])		#same for the right clade
      if (!all(sapply(expl, isExplained))) {
        #and check if all the hit groups it contributes to can still be explained. If not, we will not remove the hit group, since it is required to explain others
        clade1Sp = clade1Spb
        clade2Sp = clade2Spb		#so we restore the lists
        required[hitGroup] = T 		#mark the hit group as required
        cat(".")
        next			#and jump to the next hit group
      }
    ...
    if (any(toChange))
      clade1Sp[toChange] = removeHitGroupFrom(hitGroup, clade1Sp[toChange])
    toChange = intersect(orderedHitGroups[-(1:i)], contributedClade2[[hitGroup]])
    if (any(toChange))
      clade2Sp[toChange] = removeHitGroupFrom(hitGroup, clade2Sp[toChange])
...
httHits = httHits[, .(query,subject,q,s,sp1,sp2,f1,f2,superF,mrca,pID,length,qStart, qEnd, sStart,sEnd,ka,ks,length.aa,com,hitGroup)]		#only retains the columns we need
...
)		#correspondance between  repeat modeler super families and more common super family names, which we will use from now on. This file is provided with the scripts
...
setnames(
  retainedHits,
  c("query", "subject", "f1", "f2", "com", "superFName"),
  c(
    "copy1",
    "copy2",
    "consensus1",
...
)		#replace column names with more user-friendly ones
