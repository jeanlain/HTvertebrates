#establishes ka ks rate for pairs of TEs copies that diverged by transposition within genomes

source("HTvFunctions.R")

# we retreive blastn hits of copies against themselves (generated by TEselfBlastn.R at step 8-prepareClustering.R). As these files are very big, we firt select only the hits we need
# we will then compute Ka and Ks corresponding to pairs of copies in these hits
...
select = function(superF) {				#selects relevant blast hits of TEs to compute ka ks rates. We do it per superfamily (for big super families, several blast were performed in parallel so there are multiple output files)
	import = function(file) {			#imports a blast file of copies against themseves
		dt = fread(file, sep ="\t", header = F, drop = c(3,9), nThread = 1)
		dt = dt[V4 >= 300L]		#selects hits that are long enough for Ka Ks computations
		dt = dt[spForCopy[V1] == spForCopy[V2] & comForCopy[V1] == comForCopy[V2],]		#selects only hits between copies from the same species and community
		cat(".")							#progress indicator (the files are big, this takes time)
		dt
	}
	rbindlist(mclapply(blastFiles[[superF]], import, mc.cores = 20, mc.preschedule = F)) 			#rbinding filtered blast results from the same super family
}
...
############### to avoid unnecessary ka/ks computations, we select TE hits that cover a protein region of sufficient length (≥ 300 bp), similar to what we did in step 6
...
TEhits = TEhits[inter >= 300L, data.table(q=V1, s=V2, query=ids[match(V1, id), name], subject=ids[match(V2, id), name], qStart=V4, qEnd=V5, sStart=V6, sEnd=V7)]		#selects hits where this region is at least 300 bp, and names the columns we need. We also have to retreive original copy names for the TEKaKs.R script below (which uses a fasta files of original copy names)
...
system("Rscript TEKaKs.R TEs/TEevolution/selectedHits/selected.out TEs/TEevolution/selectedHits/blastx.out TEs/clustering/selectedCopiesKs05occ200.fas TEs/TEevolution/Ks/ 30")
