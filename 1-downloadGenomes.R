...
sp = data.table(read.table("307.species.info.txt", header =F, sep ="\t", quote = "")[,c(1,9)])		# fread() doesn't like this file, hence read.table()
links = fread("ftp_links.txt", select = c(1, 20))				#retreived from ftp://ftp.ncbi.nlm.nih.gov/genomes/genbank/vertebrate_other and ftp://ftp.ncbi.nlm.nih.gov/genomes/genbank/vertebrate_mammalian
...
f = !file.exists(sp$destination)								#it case this script needs to be re-run, avoids downloading genomes several times
