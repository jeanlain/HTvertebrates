...
# ths script uses the tabular file of htt hits with hit group identifier generated by 10-hitClusteringRound2.R
...
# STAGE ONE, we gather the TE copies present in hit groups ---------------------------------------------------------------
# this willb e our criterion to evaluate the risk of DNA contamination
# As many hits have been discarded in early stages of the analysis,
# we retreive TE copies that should belong to the varous hit groups
...
# we will save fasta files for seperate species, to make one blast for each.
...
# retreives full copy names (useful later)
...
# add useful hit information in new columns
...
# For this, we retreive the mean pID and scores per copy per hit group
...
# we create a group-copy pair indentifier
bestIDs[, pair := stri_c(hitGroup, "_", copy)]

# so as to retreive the mean pID and score for hits involving a query, for each hit group
...
# we also retreive the species to which copies belong,
...
# STAGE 2, we evaluate if hit groups should be excluded -----------------------------------------------------------
...
hitGroupStats <- httHits[, data.frame(
    ...
    ksMode(ks + 2 * vks), # the Modal class of the Ks dstribution (see HTvFunctions.R)
    N = .N, # number of hits per hitGroup
    nq = length(unique(q)), # number of "query" copies, i.e. from the left clade
    ns = length(unique(s))
), # same for copies from the right clade
...
hitGroupStats[, keep := nQ >= 5L & # it must involve at last 5 copies in the left clade
