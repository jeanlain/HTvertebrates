...
#retreive similar copies. So we blast copies against the TEs of their host
#genomes.
...
fasFiles = stri_c("TEs/clustering/testConta/",
                  names(spl),
                  ".selectedCopies.fas")
l = Map(writeXStringSet, spl, fasFiles)

system(
  'sbatch --mail-type=BEGIN,END,FAIL --cpus-per-task=15 --mem=100G --wrap="Rscript blastAgainstAllTEs 15"'
)
...
blast = blast[V3 > 75 &
                V4 >= 200L]						#removes hits of low quality since none of the retained HTT hits had pID < 75%
...
bestIDs = rbind(bestIDs , httHits[, .(mean(pID), mean(score)), by = .(hitGroup, copy =
                                                                        subject)])		#we need to do that for copies that are in "query" and "subject" columns
...
#to exlclude VT, we evaluate the distribution of Ks of TEs in a hit group, in
...
quant = Ks[Ks < 10, .(
  ...
  m = mean(Ks),
  divTime = divTime[1]
), by = clade] #collects useful information, including the 0.5% Ks quantile that we used as a threshold
...
hitGroupStats = httHits[, data.frame(
  ...
  nq = length(unique(q)),
  ns = length(unique(s))
), by = .(hitGroup, mrca, superF)]  #nq and ns are number of copies per hit group (and each of the two clades) actually involved in the hits
...
#checks if ka/ks of TEs correlates with the divergence of clades involved. It
#should not (a priori), especially for DNA transposons, unless there is some
#spurious VT for which ka/ks should be closer to 1, while ka/ks for TEs
...
p = perAge[DNA == T, plot(age,
                          kaks,
                          ylim = c(0, max(kaks)),
                          xlab = "Time since divergence (My)",
                          ylab = "Ka/Ks")]		abline(v = 120, col = "red")
dev.off()
hitGroupStats[, keep := nQ >= 5L &
                nS >= 5L &
                nq >= 2L &
                ns >= 2L &
                (nMode > nLast + 20L |
                   maxkS < qKs - 0.2) &
                age > 120]	#based on the above, we retain hit groups involving at least 5 copies in each clade (to account for possible contamination), including 2 copies per clade in the retained hits, and whose kS is low enough and involving lineages thare are quite divergent
