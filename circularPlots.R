...
angleOfPoint <- function(x) {
    # the angle of a point to draw, as in a trigonometric circle, the origin being the center of the plot area. The user is not supposed to use this function
...
convertCoords <- function(x, y) {
    # converts coordinates between a regular plot and its circularized version. The user is not supposed to use this function
    if (!exists("circ__")) {
...
circPoints <- function(x, y, ...) {
    # draws points on a circular plot (equivalent of the points() function). Points shapes are not rotated
...
circSegments <- function(x0,
                         ...
                         curved = T,
                         ...) {
    # draws segments on a circular plot. Segments will be curved unless those for which "curved" == F
...
circLines <- function(x,
                      ...
                      draw = T,
                      ...) {
    # draws lines on a circular plot
...
circPolygon <- function(x,
                        ...
                        curved = T,
                        ...) {
    # draws a polygon on a circular plot
    if (connect) {
        # done if we want the polygon to be closed, that is, if the 2 ends join (likelky at noon)
        x <- c(circ[2], x, circ[3])
        y <- c(mean(y[1], last(y)), y, mean(y[1], last(y)))
    }
    if (is.numeric(closeAtY)) {
        # used to automatically specify that the polygon should have a bottom side a closeAtY  (so we don't have to close it manually)
        x <- c(x, last(x), x[1])
