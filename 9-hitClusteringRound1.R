...
# - the self-blastn output generated by the previous stage

# the ouptut will be a tabular file of HTT hits with a new column attributing a community number to each, "occ200_comm.txt"

source("HTvFunctions.R")

# imports the HTT hits to cluster
httHits <- fread("occ200Ks05.txt")

# STEP ONE, we swap query and subject species in the TE-TE hit table -------------------------------------------------------
# An HTT Involves two clades,
# to compare TE copies wihtin clade to avaluate criterion 1, it is
...
# We assign subclade numbers (node numbers of the tree) to species in each hit, and below we ensure that the subclade with lower number is always on the left.
httHits[, c("sc1", "sc2") := .(subClades[chmatch(stri_c(sp1, mrca, sep = " "), comb), node], subClades[chmatch(stri_c(sp2, mrca, sep = " "), comb), node])]
...
# we retreive copy integer IDs
copyIDs <- fread("TEs/clustering/selectedCopiesKs05occ200.IDs.txt")
copyIDs[, name := copyName(copy)]

# adds new columns for integer ids corresponding to both copies in each hit
...
# we split "query" copies for each superfamily and mrca, which we will compare as these are from the same (left-hand) subclade
queries <- split(queries$V1, queries[, paste(superF, mrca)])

# we will sort groups by decreasing number of copies in queries (for better CPU usage)
ql <- sapply(queries, length)
queries <- queries[order(ql, decreasing = T)]

# we do the same for subjects TE copies
...
superF <- gsub("/", ".", splitToColumns(names(queries), " ", 1), fixed = T)
queries <- split(queries, superF)
subjects <- split(subjects, superF)

# and we sort these list by decrasing numbers of copies (we prefer starting with the large ones)
...
getHits <- function(superF) {
    ...
    import <- function(file) {
        ...
        # now select hits between copies of the same subclade
        hits <- Map(
            function(q, s) {

                # each hit must be between two "queries" (left-hand clade)
                hits[(V1 %in% q & V2 %in% q) |
                    (V1 %in% s & V2 %in% s), ]

                # or between two subjects (right-hand clade)
            },
            queries[[superF]], subjects[[superF]]
        ...
        hits <- hits[!duplicated(data.table(V1, V2)), ]
...
# Hits will be indentified by their row indices. We again replace slashes in super family names with periods
...
# STEP FOUR, collecting results of clustering ---------------------------------------------------------------------
