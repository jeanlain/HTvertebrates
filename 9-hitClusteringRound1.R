...
##%######################################################%##
#                                                          #
####     This stage lusters TE-TE hits iteratively      ####
####       to delinate HTT events ("hit groups")        ####
#                                                          #
##%######################################################%##


#This state is the first step where clusters hits from the same pair of "young"
#caldes: clades of less than 40 My, in which we did not even look for HTT and
#did not measure any BUSCO Ks then we will agregate the clusters that may
#reflect the same HTT. clustering and agregation will be based on criterion 1:
...
#we will only compare hits that involve the same TE super family and species
#that coalesce to the same MRCA, or else they cannot result from the same HTT,
#and there is not even the notion of a shared "clade" between hits to use
#criterion 1. to compare TE copies wihtin clade to avaluate criterion 1, if is
#easier to swap sp1 and sp2 so that sp1 always corresponds to the same clade
#(and sp2 to the other) among the two sister caldes that diverged from an MRCA.
#So we will compare "queries" (copies from the left-hand clade) with each others
...
#pIDs, since these are in the httHits table for this, we need copy integer
#identifiers as these were used in the self blast to save space
...
queries = split(queries, superF)
subjects = split(subjects, superF)

nq = sapply(queries, function(x)
  length(unique(unlist(x))))			#and we sort these list by decrasing numbers of copies (not required, but I prefer starting with the large ones)
queries = queries[order(nq, decreasing = T)]
subjects = subjects[order(nq, decreasing = T)]

blastFiles = list.files("TEs/clustering/blastn/done",
                        pattern = "",
                        full.names = T)		#path of self blast result files generated in set 8-prepareClustering.R
...
getHits = function(superF) {
  #extracts the relevant TE-TE hits for a given super family
  ...
  import = function(file) {
    ...
    hits = Map(function(q, s)
      hits[(V1 %in% q &
              V2 %in% q) |
             (V1 %in% s &
                V2 %in% s), ], queries[[superF]], subjects[[superF]])		#select hits between copies of the same subclade
    hits = rbindlist(hits)
  ...
  hits = mclapply(files,
                  ...
                  mc.preschedule = F)		#applies the function for each blast file of the super family, in parallel
  writeT(rbindlist(hits),
         stri_c(filteredHitFolder, superF, ".out"),
         col.names = F)			#writes a single file of selected hits
...
#we now cluster hits between species of the same pair of "young" clades that
#diverged within the last 40 My. Wihtin these clades, we did not even search for
#HTT (no blast) and we didn't measure Ks at buscos. These clusters of hits will
#be the initial ones (the "communities") that we will agregate in a second round
...
conv = httHits[, data.table(
  hit,
  pID = as.integer(pID * 1000L),
  group,
  query = q,
  subject = s
)]		# We get the column we need to the clusering convert blast pIDs to integers for efficiency in the clustering
...
)		#number of hits to compare 2by2 per "group"
...
perSuperF[, batch := 0L]
perSuperF[nHits > 30000, batch := 1:.N]						#attributes batches (job numbers) to super families. Note that several super families with low number of hits may be processed in the same job
...
system(
  'sbatch --mail-type=BEGIN,END,FAIL --cpus-per-task=5 --mem=100G --wrap="Rscript iterativeFirstClustering.R 6 5"'
)

###### collecting results
files = list.files("TEs/clustering/round1",
