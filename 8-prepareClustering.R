...
####### preparing the clustering of TE hits to count HTT events. 

source("HTvFunctions.R")
dir.create("TEs/clustering")
httHits = fread("occ2000ds05.txt")		#selected TE-TE hits from the previous step

###### there are too many hits for this clutering, we only retain 200 of hits per single-linkage cluster, as hits from the same cluster are very likely to represent the same HTT
...
# extracting TE copies sequences in the retained hits, as we will compare copies with super families with blastn all vs all (referred to as "self blast" afterwards). This will be used to determine of two hits represent the same HTT (identity at the hits is lower than identity between copies in each clade, refered to as "criterion 1" in the paper), and also for the anlysis of TE evolution wihtin genomes
copies = httHits[, union(stri_c(sp1,":",query, ":", f1, "#", superF), stri_c(sp2, ":", subject, ":", f2, "#", superF))]  #we add species and classification information to copy names, as specified in the fasta file of TE copies, so we can extract them
write(copies, "temp.bed")		#temporary file used by seqtk
seqtk("TEKs/selectedCopiesAA300cl2000.fas", "temp", "TEs/clustering/selectedCopiesKs05occ200.fas")
...
m = lapply(fasNames, function(x) system(paste("makeblastdb -dbtype nucl -in", x)))		#makes blast databases
...
indices = unlist(Map(function(x, y) rep(1:x, length.out = y), nComp$nt, nD))			#creates an integer index corresponding to the sub-query (max 200), for each sequences
queryParts = stri_c("TEs/clustering/blastn/split/", rep(nComp$query, nD), "_", indices, ".fas") 				#which is used to attribute each sequence to a sub-query (part)
...
dbs = gsub("split", "db", stri_extract_last(names(spl), regex = "[A-Z]+.[^_]+"))		#the databases corresponding to the split queries								
searches = data.table(query = names(spl), db = stri_c(dbs, ".fas"))	#creates a table listing the blast searches to do
searches[, out := gsub("split", "out", query)] ; searches[, out := gsub(".fas", ".out", out)]
...
system('sbatch --mail-type=BEGIN,END,FAIL --cpus-per-task=4 --mem=120G --wrap="Rscript TEselfBlastn.R 1 4"')  #example for batch 1
