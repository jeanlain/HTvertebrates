...
TEhits = fread(
  "all.quantile005score200.blastn.out",
  header = F,
  sep = "\t",
  col.names = c(
    "query",
...
copies = TEhits[, data.table(
  ...
  seq = c(query, subject),
  fam = c(f1, f2),
  superF = c(superF1, superF2)
)]		#copies are identified by species, sequence name, family and superfamily names, for both copies and subjects of the hits
...
m = Map(function(x, y)
  write(x, file = y),
  spl,
  stri_c("TEs/selectedCopies/", names(spl), ".txt"))		#copies names are saved in separate files for each species, as the blastx will be run in parallel
...
blastx = fread(
  "TEs/blastx/all.copies.successiveBlastx.out",
  header = T,
  sep = "\t",
  col.names = c(
    "query",
...
)				#the combined blastx results generated above
...
TEhitsCoveringProteins = TEhits[superF1 == superF2 &
                                  inter >= 300L]										#selects hits with sufficient protein region (300 bp) and between TEs assigned to the same super families
setnames(TEhitsCoveringProteins, "superF1", "superF")
TEhitsCoveringProteins[, superF2 := NULL]		#so now we only need one superfamily name per hit
