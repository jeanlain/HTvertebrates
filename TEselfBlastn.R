...
f = searches[, batch %in% job &
               !basename(out) %in% basename(done)]		#in case jobs need to be relaunched, we avoid redoing already completed searches
...
blastn = function(query, db, out) {
  ...
  )		#because this is a self blast, we select results where the query name (an integer) is < subject name,
  #to reduce output file size by two (we only retreive the hit copy1 vs copy2, not the reverse).
