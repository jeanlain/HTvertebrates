...
forw = pairs[sp1 < sp2]
rev = pairs[sp1 > sp2]			#we will process forward and reverse blast searches (as each species was used as query and db) simultaneously, so as to select the best hit among the two reciprocal ones
...
filterHits = function(out, rev, filtered, q) {
        ...
        f2 = stri_c(mat2[, 1], "-", mat2[, 6])		#creates TE family names from species names (first field) and family names
        f = !f1 %chin% toRemove &
                !f2 %chin% toRemove							#so we can filter-out hits involving dubious TEs families
        if (sum(f) == 0) {
                file.create(filtered)
                return(NULL)
        }					#again creates an empty output file if there is no retained hit
        mat1 = rbind(mat1[f, ])
        mat2 = rbind(mat2[f, ])							#rbind() makes sure that these will be matrices and not vectors, even if there is only one hit (to avoid a dimension problem)
        ...
        fwrite(
                ...
                quote = F,
                sep = "\t"
        )		#write file of filtered hits (not returned by the function, to save memory and because Map cannot apparently return tables)
        c(basename(filtered), nr, nr2, nrow(blast))			#instead we return the initial number of hits and number of retained hits
}


m = pairs[!file.exists(filtered), mcMap(filterHits,
                                        out,
                                        rev,
                                        filtered,
                                        q,
