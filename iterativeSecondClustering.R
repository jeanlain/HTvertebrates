...
#applies cliterion 1 to hits from different communities, to potentially merge them into hit group (htt events). This shares a lot of codes from the first clustering script
#many of the commands replicate what we did in round 1, but there are subtleties. 
...
communityPairStats = function(superFam) {
	...
	blast = fread(blastFile, sep ="\t", header = F, drop = 4)
	...
	processHitsOfGroup = function(group) {
		...
		sub = blast[V1 %in% uCopies & V2 %in% uCopies, .(copyPair, V3)]		#we selected the TE hits involving these copies
		...
		if(nBatches > 1) hitBatches = splitEqual(hitBatches[[1]], n = nBatches)
		criterion_1 = function(hitBatch) {			#this "connects" hits according to the criterion, similar to what we did in the first round
			pairs = data.table(V1 = rep(hitBatch, nHits-hitBatch), V2 = unlist(lapply(hitBatch[1]:max(hitBatch)+1L, function(hit) hit:nHits)))					#makes all possible pairs of hits for this batch
			...
			pairs[,c("intra1", "intra2") := .(sub[match(qPair, copyPair, nomatch = .N), V3], sub[match(sPair, copyPair,nomatch =.N), V3])]			#so we can retreive their identities. The nomatch argument makes it so that the pID of the last row in sub (wich has pID 0) is return. This seeds things up a little
			pairs[q1 == q2, intra1 := 100000L] ; pairs[s1 == s2, intra2 := 100000L] ; 		#if copies are the same, they are not in the blast hits so we set their identity to 100%
			pairs[,maxIntra := pmax(intra1, intra2)]
			pairs[, connected := inter1 < maxIntra | inter2 < maxIntra]			#the 2 hits will be connected in the best intra-clade identity is higher than one inter-clade identity (that of hits)
			cat("*")
			pairs[,.(allDiff=sum(!connected), diff=sum(maxIntra > 0L & !connected), valid = sum(maxIntra >0L), tot=.N), by = .(com1, com2)]		#returns statistics for each community pair (note that a pair may also involve the same community twice, which helps to assess how much hits within a community are connected)
			#allDiff is the number of pairs of hits not passing the criterion, diff is the same, but for hits whose copies have some degree of intra-clade indentity, valid is the number of pairs of hits whose copies have non-zero intra-clade indentity, tot is the number of pairs of hits evaluated
		}
		statsPerCom = rbindlist(mclapply(hitBatches, criterion_1, mc.cores = min(nCPUs, length(hitBatches)), mc.preschedule = F))
		statsPerCom[,.(allDiff=sum(allDiff), diff=sum(diff), valid = sum(valid), tot=sum(tot)), by = .(com1, com2)]
	}
	stats = rbindlist(lapply(groups, processHitsOfGroup))
	writeT(stats, stri_c("TEs/clustering/round2/", superFam, ".all.txt"))			#writes to disk for safety, this is also returned by he function
