...
#                                                          #
## %######################################################%##
# This scripts is lauchned from 10-hitClusteringRound2.R and uses files generated by that script
...
communityPairStats <- function(superFam) {
    ...
    processHitsOfGroup <- function(group) {
        # we retreinve the hits of the group
        hits <- copy(group)
        ...
        # to create batches that will analyze 2^28 pairs of hits at once
        ...
        criterion_1 <- function(hitBatch) {
            ...
            pairs[, c(
                ...
                "com1", # community id
                "inter1", # the inter-clade indentity (representing the HTT)
                "q2", "s2", "com2", "inter2"
            ) # and the equivalent for hit2
            := data.table(
            ...
            # so we can retreive within-clade sequence identities.
            # The nomatch argument below makes it so that the pID of the last row
            # in selectedHits (wich has pID 0) is return where there is no match (no hit).
            ...
            pairs[, .(
                ...
                diff = sum(maxIntra > 0L & !connected), # the same, but for hits whose copies have some degree of intra-clade indentity
        ...
        # as the satistic returned by criterion_1() were obtained for
