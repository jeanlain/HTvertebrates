#finds homologies between TE copies belong to different hit groups, which we use to infer the minimal number of HTT events to explain these groups
...
blastFiles = list.files("TEs/clustering/blastn/done", pattern = ".out", full.names = T)            #blast files of copies against themselves
...
hitGroupPairStats = function(superF) {
        ...
        import = function(file) {                       #imports a blast file of copies against themseves
                dt = fread(file, sep ="\t", header = F, select = c(1:3,9), nThread = 2)
                dt = dt[V1 %in% copies & V2 %in% copies]
                dt[,V3 := as.integer(V3*1000)]          #converts pID to integer for speed and memory reasons
                dt  
        }
        dt = rbindlist(mclapply(blastFiles[[superF]], import, mc.cores = 5, mc.preschedule = F))
        dt = rbind(dt[,.(V1, V2, V3)], dt[,.(V1 = V2, V2 = V1, V3)], data.table(V1 = copies, V2 = copies, V3 = 100000L))            #duplicate the hits by creating reciprocal ones (used below) and adds self hits with 100% pIDs
        ...
        hitGroupLinks = function(copies, hitGroup1) {                   #for copies in each hit group (named hitGroup1), this retreives the hit groups with homologous copies
                ...
                pairs = pairs[, data.table(copy = rep(copy, len), pID = rep(pID, len), hitGroup2)]          #places all this in a data.table
        ...
        hitGroupPairs = Map(hitGroupLinks, copyPerHitGroup, as.integer(names(copyPerHitGroup)))         #applies the above for all hit groups of the super family
       	writeT(rbindlist(hitGroupPairs), stri_c("TEs/clustering/networks/", superF, ".hitGroupPairCopies.txt"))
		rbindlist(hitGroupPairs)
}

res = mclapply(names(hitList), hitGroupPairStats, mc.cores = nCPUs, mc.preschedule = F)
writeT(rbindlist(res), stri_c("TEs/clustering/networks/all.hitGroupPairCopies.txt"))
