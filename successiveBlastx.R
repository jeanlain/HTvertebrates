# blasts TE copies against repbase proteins to find protein regions in TEs. As a single copy may encompass several proteins (in particular, for retrotransposons), we blast copies once to retain only the best hit per copy, then we extract TE parts that are not in HSPs, and blast them again. Five "rounds" of blast searches are performed. We do that rather than retrieving many hits per TE in a single blast because this gives not garanty to find all the porteins of the TE (most of the hits would be likely to inolve the same TE region that aligns on homologous proteins of repbase).
...
successiveBlastx  = function(sp, done, maxRounds) {				#does the blastx searches for a given species. maxRounds will be 5
...
m = mcMap(successiveBlastx, sp, done, 5, mc.cores = nCPUs, mc.preschedule = F)		#performed in parallel for several species
